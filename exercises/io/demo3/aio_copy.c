 ...
 * gcc -O2 -o aio_copy aio_copy.c -lrt
 * In this example the input file (read synchronously) is copied to the output file (asynchronously) using the specified transfer size. A signal handler counts the number of completions, but is not required for the functioning of the program. A call to the aio_suspend function is sufficient.
 */
